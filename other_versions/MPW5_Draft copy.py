    ...
    while True:
        try:
            ...
            if first_option == 0:
                ...
                rows = cursor.fetchall()
                for row in rows:
                    print(f'OrderID: {row[0]}, CustomerName: {row[1]}, CustomerAddress: {row[2]}, CustomerPhoneNumber: {row[3]}, OrderStatus: {row[4]}, FinalOrder: {row[5]}, CourierName: {row[6]}, OrderDateTime {row[7]}')
                cursor.close()
            else:
                os.system('cls')
